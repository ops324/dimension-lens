 ...
 *
 * ## **訂正 —— 「門が全開なら厳密にゼロ」は偽だった**（Phase 2c-vi で実測）
 *
 * 上の表のすぐ下に、2c-iv はこう書いていた:
 *
 * > **門が全開の領域では後退は厳密にゼロ**である。
 *
 * **その表は全開域のうち `d ≥ 2.0` しか採っていない。** 全開域は `cloudWeight(d) === 1` の全体で、
 * 下側の `[0, 1.55]` を**1 点も含んでいなかった**。全開の 13 点を 18000 フレーム（300 秒）
 * 回すと、下側に反例が 2 つ在る:
 *
 ...
 * （独立監査が切り分けた: K = 32 / 1024 / 8192 / 全 89,208 点が**ビット一致**し、
 * M を 65536 にしたときだけ動く。`need` 換算の不足は 21 スロット中 `d = 1.25` が最悪で **99.700%**）。
 * `core/framingEnvelope.ts` の収束表は `d = 3` で作られており、**1.25 はそこにも
 * 「`d = 0.5〜5` の 10 点」にも入っていない**。
 *
 * **これは 2c-iv が書いた反省をその PR 自身が破っていた例である** ── あの PR は
 * 「標本を 1 枚しか通さない測定は『測った』と書く前に標本を増やす」と §8 に書きながら、
 * 今度は **`dimLevel` を偏って採った**。規律の文章では止まらない。
 * → `npm run ladder:budgets`（予算の歯）と `survival.test.ts`（配線の生存）を Phase 2c-vi で
 * 機械にした。掃引の密度を機械で見る `--density` は次フェーズ。
 */
