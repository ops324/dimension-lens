 ...
 * ## 載せられない表が在る（費用・実測）
 *
 * | 対象 | `npm test` の増分 | `npm run teeth` 換算 |
 * |---|---|---|
 * | 純関数だけ（`halfFloat` / `blendModel` / `spriteGain`） | +0.02 s | +1.1 s |
 * | ＋ 包絡 `M ≤ 65536`（この台帳が採るところ） | **+3.4〜4.1 s** | **+3 分 17 秒〜4 分 4 秒** |
 * | ＋ `layout()` を含む反復軸（18000F / 36000F 系） | +464 s | **+6.96 時間** |
 *
 * **真ん中の行は初稿で 2.4 倍外していた**（`+1.5 s` / `+81 s` と書いた）。理由が 2 つある ──
 * (1) 独立監査の指摘で**感度検査を全セルへ広げた**ので、このファイル単独の時間が
 * 1.46 s から **6.0 s** になった。(2) `npm test` の**壁時計**では増分が雑音に埋もれる
 * （10 コアのこのホストでは `widen.test.ts` の 11.6 s の陰に入る）ので、そちらを見て
 * 「安い」と判断していた。**`teeth` は vitest を 58 回**回すので、
 * 壁時計に出ない CPU がそこで 58 倍になる。
 *
 * **実測（CI・ubuntu-latest 4 コア）**: `teeth` の段が
 * 2c-viii の **16 分 15 秒 / 55 走行 = 17.7 s** から
 * 2c-ix の **20 分 26 秒（21.1 s）と 21 分 07 秒（21.8 s）** へ。**1 走行あたり +3.4〜4.1 s**。
 *
 * **幅で書くのは、同じコミットの 2 回の走行が 41 秒（3%）割れたから**である。
 * **2c-viii の側は 1 走行しか観測がない**ので、この差の一部が runner の個体差である
 * 可能性は排除できていない（§0.1 の「標本を 1 枚しか通さない測定は『測った』と書けない」）。
 * **`npm test` 単体の壁時計だけを見て「載せられる」と判断してはいけない**
 * （§7.9.2.1 が 2c-vii で書いた当のことを、この節の初稿がもう一度踏んだ）。
 *
