...
describe('シーンへの配線（node で回す）', () => {
   ...
   */
  /**
   * **題を `d = 3` に狭めた**（Phase 2c-vi）。2c-ii〜2c-v の題は
   * 「**門が全開なら**、カメラ距離は 600 フレーム回しても 1 ビットも動かない」だったが、
   * 測っているのは `d = 3` の 1 点だけである。全開域を 13 点 × 18000 フレーム回すと
   * **`d = 1.25` と `d = 1.50` が 153〜154 秒で動く**（×1.0004。`core/fit.ts` に表を置いた）。
   * 原因は `ENVELOPE_PHASES = 4096` の不足で、候補数ではない。
   *
   * **題が掃引より広いと、通ったことが主張の証拠に見えてしまう。**
   */
  it('d = 3 では、カメラ距離は 600 フレーム回しても 1 ビットも動かない', () => {
