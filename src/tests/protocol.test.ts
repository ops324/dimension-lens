...
function sampleIngested(): IngestedResponse {
...
describe('取り込みプロトコル', () => {
  ...
  it('transferablesOf は応答の形から転送リストを導出する', () => {
    ...
    expect(transferablesOf(failure(1, 'internal', 'x'))).toHaveLength(0);
  ...
  it('複製できない値が混ざると DataCloneError で落ちる', () => {
    const bad = { ...failure(1, 'internal', 'x'), onDone: () => 0 };
    expect(() => structuredClone(bad)).toThrow(/DataCloneError|could not be cloned/i);
  });

  it('失敗は code で分岐でき、message には分岐を置かない', () => {
    const f = failure(3, 'too-many-pixels', '画像が大きすぎます（99999×99999）。');
