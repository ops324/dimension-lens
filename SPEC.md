...
> 版: **Phase 1b 時点**（潰し —— [#12](https://github.com/ops324/dimension-lens/pull/12) マージ済み）
> 公開: <https://ops324.github.io/dimension-lens/> —— **標本 No.0 が描かれている。**
> `dimLevel = 2` の板経路は忠実性ラダーを ΔE00 **0.0000** で通っている（§7.6）。
> **`dimLevel` は `[0,5]` の全域で忠実性の予算内にある**（§7.7）。スライダー等の UI は Phase 3。
...
| **潰しの忠実性（`[0,5]` 全域）** | **A**（Phase 1a-iii では C） | `d=1` の列平均線 ΔE00 **0.1234**、`d=0` の平均色 **1.5244**（予算 3.0）。§7.7 |
| **`gainFor` が全域で有限** | **A**（前は `Infinity` を返しえた） | `ksumLookup` の下端分岐が `ksum = 0` を返す領域があった。`MIN_CALIBRATED_S0 = 0.86` でクランプ（§4.18） |
| `dimLevel = 0` の **−6.73%** の機構 | **C**（欠損が振幅依存であることは **A**） | 幾何では説明できない（径を逆算すると 7.28/8.45/8.49 とばらつく）。half-float の非正規数を落とすモデルは合計を 0.6% で当てるが、直接観測はしていない。§7.7 |
...
### 7.1 vitest（Phase 1b 時点で 276 件・全緑）
...
Phase 1b 以降で追加: プレートの不透明契約と語彙の規律（Phase 3）。
...
| **G9** | `d = 0` 平均色 | 図の中心が `stats.meanHex` か | **ΔE00 ≤ 3.0**（§4.6） | **1.5244**（−6.73%） |
...
### G9 の −6.73% は、予算内だが**機構が分かっていない**（水準 C）

説明を 3 つ試して、どれも当たっていない:

| モデル | 予測 |
|---|---|
| 等しい δ の half-float 累積 | **+1.08%**（符号が逆） |
| 実際の不等な δ（378 バンドの実測色）での累積 | **−0.02%**（ほぼゼロ） |
| f16 の**非正規数を 0 に落とす**モデル | **−7.37%**（実測との差 0.60%） |

3 番目が合うが、**flush-to-zero を直接観測したわけではない。** 言えるのは:

- **A**: 欠損は半径とともに単調に増える（中心 0.944 → 縁 0.812）
- **A**: **幾何の変化では説明できない** —— 3 点からスプライト径を逆算すると
  7.28 / 8.45 / 8.49 とばらつく（`gl_PointSize` の整数丸めなら一致するはず）
- **B**: 加算深度と 1 回あたりの書き込みの大きさが効いている ──
  **同じ幾何・同じ `gain` で深度だけが違う** `d=1`（深度 4）と `d=0`（深度 378）が
  −0.53% と −6.73% に分かれる。1 回あたりの書き込みは **254 倍**違う
- **C**: 正確な機構

Phase 2 の bloom は低振幅の加算を大量に積むので、**そこで再測定する**。
...
| 2 | 光の質（bloom/grade 再測定・**`CompressPass` の配線**・quality・光過敏・**G9 の −6.73% の機構**・**フレーミングの単調収束の見直し**） | |
