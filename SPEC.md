...
> 版: **Phase 1c 時点**（取り込みの頑健性と空状態 —— [#14](https://github.com/ops324/dimension-lens/pull/14) マージ済み）
> 公開: <https://ops324.github.io/dimension-lens/> —— **標本 No.0 が描かれている。**
> `dimLevel = 2` の板経路は忠実性ラダーを ΔE00 **0.0000** で通っている（§7.6）。
> **`dimLevel = 0` は予算を外している**（ΔE00 **4.3228** / 予算 3.0）── 1b が「1.5244 で予算内」と
> 書いていたのは**再現しない**。`main` でも同じ画素が出ることを確かめた（§7.7）。Phase 2 の対象。
...
| ~~潰しの忠実性（`[0,5]` 全域）が予算内~~ | **偽**（Phase 1c で実測） | `d=0` の平均色は **ΔE00 4.3228**（予算 3.0）で**外している**。1b の「1.5244」は再現しない ── `main`（6b52738）でも同じ 9×9 の画素場が出る（§7.7）|
| **潰しの忠実性（`d ≥ 0.5`）** | **A** | `d=1` の列平均線 ΔE00 **0.1234**、`d=0.5` **0.1528**。外しているのは `d=0` だけである |
| **`gainFor` が全域で有限** | **A**（前は `Infinity` を返しえた） | `ksumLookup` の下端分岐が `ksum = 0` を返す領域があった。`MIN_CALIBRATED_S0 = 0.86` でクランプ（§4.18） |
| `dimLevel = 0` の欠損の**大きさ** | **A**（1b の −6.73% は**誤り**） | 中心の峰 sRGB **129**（平均色 141 に対しリニア比 **−17.58%**）。フレーム数に依らず、後処理を全部入れても 131 止まり。§7.7 |
| `dimLevel = 0` の欠損の**機構** | **C** | 幾何では説明できない（径を逆算すると 7.28/8.45/8.49 とばらつく）。半精度の非正規数を落とすモデルも**実測 −17.58% には合わない**（1b の −7.37% は −6.73% に合わせた計算だった）。Phase 2 |
| **なぜ 1b が 1.5244 を記録したか** | **C** | 分からない。アンカー（`s0`/`gain`/`sampleWeight` 0.0058472/加算深度 378）は 1b と厳密一致しており、入力側に差は無い |
...
| **歯が抜けていないことを CI が見張る** | **A**（1b までは手作業） | `npm run teeth` が台帳の **22 変異**を入れて本当に落ちるかを確かめる。CI 実測 22/22（§7.9）|
...
**高 dimLevel で点が重なる領域は未測定**である。配線と測定は Phase 2。
...
**残る未測定は Gecko（Firefox）だけ**である。そして 1b までこの節が書いていた
...
`localStorage`（品質ティア 1 項目）→ **Phase 2**、
...
### 7.1 vitest（Phase 1c 時点で 322 件・全緑）
...
Phase 1c 以降で追加: プレートの不透明契約と語彙の規律（Phase 3）。
...
Phase 1b で **`resetRotation`**（位相を厳密に 0 へ。§7.7 の「凍結したから再現可能は偽」）を
追加。`sceneStats` も 1b で `buffer` / `sampleWeight` / `additionDepth` / `needDistance` /
...
| **G9** | `d = 0` 平均色 | 図の中心が `stats.meanHex` か | **ΔE00 ≤ 3.0**（§4.6） | ❌ **4.3228**（峰 sRGB 129・リニア −17.58%）**予算超過** |
...
### G9 は**予算を外している** —— そして 1b が記録した数値は再現しない（Phase 1c で実測）
...
品質ゲート**5 点**: `npm test` 全緑（CI）/ `npm run build` 成功（CI）/
**`npm run teeth` 全噛み（CI）** / 実ブラウザ測定の数値を PR に貼る / コンソールエラーゼロ。
...
| 2 | 光の質（bloom/grade 再測定・**`CompressPass` の配線**・quality・光過敏・**G9 の予算超過（ΔE00 4.3228・峰 sRGB 129）を直すか予算の根拠を書き直す**・**フレーミングの単調収束の見直し**・`localStorage` の品質ティア） | |
